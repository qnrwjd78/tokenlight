...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/train_tokenlight_pbr_zero3.json}"
...
ACCELERATE_CONFIG="${ACCELERATE_CONFIG:-configs/accelerate_zero3.yaml}"
...
echo "TokenLight PBR ZeRO-3 train config: ${TRAIN_CONFIG}"
echo "Accelerate config: ${ACCELERATE_CONFIG}"
echo "GPU devices: ${GPU_DEVICES:-accelerate default}"
echo "Num processes / GPUs: ${NUM_PROCESSES}"

accelerate launch "${accelerate_args[@]}" model/train_tokenlight_pbr_zero3.py \
  --config "${TRAIN_CONFIG}" \
  "$@"
