...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch TokenLight Wan inference from a JSON config.")
    parser.add_argument("--config", required=True, help="Path to a JSON launch/inference config.")
    parser.add_argument("--dry-run", action="store_true", help="Print the command without running it.")
    parser.add_argument(
        "extra_infer_args",
        nargs=argparse.REMAINDER,
        help="Optional args appended after the JSON infer_args. Use `--` before them.",
    )
...
def build_command(config: dict[str, Any]) -> tuple[list[str], dict[str, str], Path]:
    ...
    command = [python_bin, str(config.get("infer_script", "scripts/infer.py"))]
    append_cli_args(command, dict(config.get("infer_args", {})))
...
def main() -> int:
    ...
    extra = list(args.extra_infer_args)
