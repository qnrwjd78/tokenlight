...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/train_tokenlight_single.json}"
...
echo "TokenLight single train config: ${TRAIN_CONFIG}"
echo "Accelerate config: ${ACCELERATE_CONFIG}"
echo "GPU devices: ${GPU_DEVICES:-accelerate default}"
echo "Num processes: ${NUM_PROCESSES}"

accelerate launch "${accelerate_args[@]}" model/train_tokenlight_single.py \
  --config "${TRAIN_CONFIG}" \
  "$@"
