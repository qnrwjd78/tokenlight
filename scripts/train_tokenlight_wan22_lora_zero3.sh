...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/train_tokenlight_zero3.json}"
...
echo "TokenLight ZeRO-3 train config: ${TRAIN_CONFIG}"
echo "Accelerate config: ${ACCELERATE_CONFIG}"
echo "GPU devices: ${GPU_DEVICES:-accelerate default}"
echo "Num processes / GPUs: ${NUM_PROCESSES}"

accelerate launch "${accelerate_args[@]}" model/train_tokenlight_zero3.py \
  --config "${TRAIN_CONFIG}" \
  "$@"
